...
import { checkArgumentCount, checkNew, createError, INVALID_ARGUMENT } from '../utils/errors'
import { bytesToUtf8String, stringToUtf8Bytes } from '../utils/utf8'
...
import {
  bytesToHex, concatBytes, fromTwosComplement, getAddress, hexToBytes,
  isAddress,
  padLeft,
  toBigNumber, toHex, toTwosComplement
} from '../utils/utils'
...
function _decodeDynamicBytes(data: Uint8Array, offset: number, localName: string): DecodedResult {
  ...
  try {
    ...
    throw createError('dynamic bytes count too large', INVALID_ARGUMENT, {
      arg: localName,
      coderType: 'dynamicBytes',
...
class CoderDynamicBytes extends Coder {
  ...
  encode(value: Arrayish): Uint8Array {
    try {
      ...
      throw createError('invalid bytes value', INVALID_ARGUMENT, {
        arg: this.localName,
        coderType: 'bytes',
        value: error.value
      })
